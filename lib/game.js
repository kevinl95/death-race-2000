...
function onNewGremlin (data) {
  ...
  // Send existing gremlins to the new gremlin
  var i, existingGremlin
  for (i = 0; i < gremlins.length; i++) {
    existingGremlin = gremlins[i]
...
function onNewGrave (data) {
  ...
  // Broadcast new grave to connected socket clients
...
function updateTime (data) {
  ...
  if (time == 0) {  // Reset game
    time = 91
    this.broadcast.emit('update time',  { time: time })
    gremlins.forEach((enemy) => {
      this.broadcast.emit('remove gremlin', {id: enemy.id})
    });
    graves.forEach((grave) => {
      this.broadcast.emit('remove grave', {id: grave.id})
    });
    bestscore = 0
