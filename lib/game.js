...
function onRemoveGremlin (data) {
  ...
  // Gremlin not found
  if (!removeGremlin) {
    console.log('Gremlin not found in lib: ', data.id)
    return
  }
...
function onNewGremlin (data) {
  ...
  // Send existing players to the new gremlin
  var i, existingGremlin
  for (i = 0; i < gremlins.length; i++) {
    existingGremlin = gremlins[i]
    this.emit('new gremlin', {id: existingGremlin.id, x: existingGremlin.getX(), y: existingGremlin.getY(), angle: 0})
  }
...
function onNewGrave (data) {
  ...
  // Send existing players to the new grave
  var i, existingGrave
  for (i = 0; i < graves.length; i++) {
    existingGrave = graves[i]
    this.emit('new grave', {id: existingGrave.id, x: existingGrave.getX(), y: existingGrave.getY(), angle: 0})
  }
...
function onMoveGremlin (data) {
  ...
  // Gremlin not found
  if (!moveGremlin) {
    console.log('Gremlin not found in lib: ' + data)
    return
  }
...
function onMoveGrave (data) {
  ...
  // Gremlin not found
  if (!moveGrave) {
    console.log('Grave not found in lib: ' + data)
    return
  }
